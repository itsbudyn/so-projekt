...
def create_frames():    # Pobieranie wartości liczbowej, która mówi na ilu ramkach będziemy pracować
    try: 
        ...
        print("Wartość nie była liczbą całkowitą (int)!")    # Zakończenie programu w przypadku podania wartości innej niż int
        return
    except AssertionError:
        print("Wartość nie może być mniejsza od zera!")     # Zakończenie programu w przypadku podania wartości mniejszej bądź równej 0
        return
    return frames
...
def create_calls_manual():     # Tworzenie tablicy odwołań
    ...
    try: page_calls=list(map(int,str(input("Podaj odwołania: ")).split(" ")))       # Dzielenie podanych wartości przez spacje i wpisywanie ich jako int
        ...
        print("Jedna z wartości nie była liczbą całkowitą (int)!")  # Zakończenie programu w przypadku wartości innej niż int 
        return
...
def create_calls(manual:bool):
    if manual: return create_calls_manual()
    else: 
        while True:
            try:
